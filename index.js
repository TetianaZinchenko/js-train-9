...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = Array.from(iter).filter((value) => value > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  // За допомогою методу map() поsplice()двоюємо кожен елемент масиву.
...
function capitalizeWords(arr) {
  ...
  //Повертаємо слово
  // Повертаємо новий масив зі словами, у яких перша літера у верхньому
...
function findLastNegativeNumberIndex(arr) {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
  // Використовуємо метод findLastIndex()findLastIndex() для знаходження індексу останнього входження від'ємного числа
